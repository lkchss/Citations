...
`configs/economics_baseline_exposure.json`; the citation reconciliation is
documented separately as a known source-measure discrepancy.

The first panel diagnostics are generated by
`scripts/describe_exposure_panel.py` and stored beside the live panel on the
SSD. The panel has no invalid timing rows or negative exposures; annual outcome
